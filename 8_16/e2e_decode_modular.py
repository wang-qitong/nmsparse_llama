...
print(f"  稀疏化范围: 所有 32 层的 QKVO + MLP (7个投影/层, 8:16稀疏模式)")
...
if PRUNE_METHOD == "magnitude":
    pruned_cache_file = "/wangqitong/sparse_params_sparsity0.5_bank16_mlp_qkvo_8_16.pt"
else:
    pruned_cache_file = "/wangqitong/sparse_params_sparsity0.5_bank16_mlp_qkvo_8_16_sparsegpt.pt"
...
if os.path.exists(pruned_cache_file):
    ...
    if PRUNE_METHOD == "sparsegpt":
        ...
        print("  - 请先运行: python 8_16/prune_sparsegpt_cache.py")
        sys.exit(1)
...
def test_sparse():
    ...
    for layer_idx in range(num_layers):
        ...
        for proj_name in ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]:
            ...
            w = pruned_weight if pruned_weight.is_cuda else pruned_weight.to(DEVICE)
            proj_module.weight.data = w.T.contiguous()
    print("✓ 剪枝后权重已加载")
...
def test_dense_original():
    ...
    for layer_idx in range(num_layers):
        ...
        for proj_name in ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]:
            ...
            w = original_weight if original_weight.is_cuda else original_weight.to(DEVICE)
            proj_module.weight.data = w.T.contiguous()
        layer.self_attn.forward = original_block_forwards[layer_idx]["self_attn"]
        layer.mlp.forward = original_block_forwards[layer_idx]["mlp"]
...
if __name__ == "__main__":
    ...
    output_dir = "/wangqitong/8_16/logs_8_16"
