...
# ===== 加载或生成剪枝权重 =====
pruned_cache_file = f"/wangqitong/sparse_params_sparsity0.5_bank64_mlp_qkvo_32_64.pt"
...
def test_sparse():
    ...
    print(f"\n【所有 {NUM_TEST_ITERATIONS} 次测试时间（Wall）】")
    for i, t in enumerate(all_wall_times_ms, 1):
        print(f"  第 {i:3d} 次: {t:.3f} ms")
    ...
    if PROFILE_COMPONENTS:
        result.update({
            'qkvo_ms': float(qkvo_ms),
            'attn_core_ms': float(attn_core_ms),
            'mlp_ms': float(mlp_ms)
        })
...
if __name__ == '__main__':
    ...
    output_dir = "/wangqitong/32_64/logs_32_64"
