...
def load_and_prune_weight(
    ...
    W = weight_module.weight.data.cpu().clone()  # [out, in]
    ...
    topk_idx = torch.topk(real_weight_3d.abs(), k=num_keep_per_bank, dim=1, largest=True, sorted=True).indices
...
def convert_to_sparse_format(
    ...
    nonzero_local_idx = torch.topk(mask_3d.to(torch.float32), k=num_nonzeros_per_bank, dim=1, largest=True, sorted=False).indices
    nonzero_local_idx = nonzero_local_idx.sort(dim=1).values

    selected_data = weight_3d.gather(dim=1, index=nonzero_local_idx)
    bank_offsets = (torch.arange(num_banks, device=weight_3d.device, dtype=nonzero_local_idx.dtype) * num_bank_val).view(
        num_banks, 1, 1
    )
