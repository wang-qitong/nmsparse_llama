...
sys.path.insert(0, os.path.join(_PROJECT_ROOT, "8_16"))
from sparsegpt_impl.sparsegpt import SparseGPT


LLAMA_MODEL_PATH = "/wangqitong/llama_2-7b"
DEFAULT_CACHE_FILE = "/wangqitong/sparse_params_sparsity0.5_bank32_mlp_qkvo_16_32_sparsegpt.pt"
...
def _load_wikitext2_train_tokens(tokenizer, *, max_examples: int) -> torch.Tensor:
    try:
    ...
    if os.path.exists(LOCAL_WIKITEXT2_PATH):
        data_files = {"train": f"{LOCAL_WIKITEXT2_PATH}/train-00000-of-00001.parquet"}
...
def _validate_nm(mask: torch.Tensor, *, num_bank_val: int, keep: int):
    if mask.is_cuda:
        mask = mask.cpu()
    K, N = mask.shape
    if K % num_bank_val != 0:
    ...
    if not torch.all(nnz == keep):
        ...
        raise ValueError(
            f"N:M validation failed at bank={bank_id} col={col_id}: expected keep={keep}, got {actual}"
        )


def _make_fixed_keep_mask(weight: torch.Tensor, *, num_bank_val: int, keep: int) -> torch.Tensor:
    if weight.is_cuda:
        weight = weight.cpu()
    K, N = weight.shape
    if K % num_bank_val != 0:
    ...
    w3 = weight.reshape(nb, num_bank_val, N)
    m3 = (w3 != 0)
    nnz = m3.sum(dim=1)

    too_many = torch.nonzero(nnz > keep, as_tuple=False)
    if too_many.numel() > 0:
        bank_id = int(too_many[0, 0].item())
        col_id = int(too_many[0, 1].item())
        actual = int(nnz[bank_id, col_id].item())
        raise ValueError(
            f"N:M mask has too many nonzeros at bank={bank_id} col={col_id}: expected keep={keep}, got {actual}"
        )

    need_fill = torch.nonzero(nnz < keep, as_tuple=False)
    for bi, ci in need_fill.tolist():
        bi = int(bi)
        ci = int(ci)
        need = int(keep - nnz[bi, ci].item())
        if need <= 0:
            continue
        cand = torch.nonzero(~m3[bi, :, ci], as_tuple=False).flatten()
        if int(cand.numel()) < need:
            raise ValueError(
                f"not enough candidates to fill N:M mask at bank={bi} col={ci}: need={need} cand={int(cand.numel())}"
            )
        fill_idx = cand[:need]
        m3[bi, fill_idx, ci] = True

    return m3.reshape(K, N).to(torch.float32)
    ...
    *,
    ...
    for layer_idx in range(len(layers)):
        ...
        for name in PROJ_NAMES:
            ...
            mask = _make_fixed_keep_mask(pruned_kn, num_bank_val=num_bank_val, keep=keep)
            _validate_nm(mask, num_bank_val=num_bank_val, keep=keep)
            ...
            if mat_data_cpu.shape != (num_bank * num_nonzeros_per_bank, N):
                raise RuntimeError(
                    f"mat_data shape mismatch: got={tuple(mat_data_cpu.shape)} expected={(num_bank * num_nonzeros_per_bank, N)}"
                )
            if mat_index_cpu.shape != (num_bank * num_nonzeros_per_bank, N):
                raise RuntimeError(
                    f"mat_index shape mismatch: got={tuple(mat_index_cpu.shape)} expected={(num_bank * num_nonzeros_per_bank, N)}"
                )
        ...
        if (layer_idx + 1) % 4 == 0:
            print(f"  ✓ Layer {layer_idx} pruned (SparseGPT N:M={keep}:{prunem})")
...
def main():
    ...
    parser.add_argument("--prunen", type=int, default=16)
    parser.add_argument("--prunem", type=int, default=32)
    parser.add_argument("--percdamp", type=float, default=0.01)
    parser.add_argument("--blocksize", type=int, default=128)
    parser.add_argument("--num_bank_val", type=int, default=32)
