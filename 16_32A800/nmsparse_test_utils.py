...
def convert_to_sparse_format(
    ...
    if selected_index.numel() > 0:
        min_idx = int(selected_index.min().item())
        max_idx = int(selected_index.max().item())
        if min_idx < 0 or max_idx >= K:
            raise ValueError(f"mat_index out of bounds: min={min_idx} max={max_idx} K={K}")

        local_from_global = selected_index - bank_offsets
        min_local = int(local_from_global.min().item())
        max_local = int(local_from_global.max().item())
        if min_local < 0 or max_local >= num_bank_val:
            raise ValueError(
                f"mat_index violates bank-local range: min_local={min_local} max_local={max_local} num_bank_val={num_bank_val}"
            )
