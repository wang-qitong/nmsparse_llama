...
if os.path.exists(pruned_cache_file):
    ...
    # 要求：
    #   - vec_width = NUM_BANK_VAL (每个 bank 的 K 宽度)
    #   - block_width = 每个 bank 的非零数 = int(NUM_BANK_VAL * (1 - SPARSITY))
    #   - w = block_width * num_bank (总非零行数)
...
def apply_sparse_forwards(model, nmsparse_module, sparse_params, proj_names, timer_events=None):
    for layer_idx in range(len(model.model.layers)):
        layer = model.model.layers[layer_idx]
        for proj_name in proj_names:
            proj_module = get_projection_module(layer, proj_name)
...
def test_sparse():
    ...
    # ===== Correctness check: sparse vs dense_pruned (1 layer + 1 Linear) =====
    try:
        check_layer_idx = int(os.getenv("E2E_CHECK_LAYER", "0"))
        check_proj_name = os.getenv("E2E_CHECK_PROJ", "q_proj")
        check_batch = int(os.getenv("E2E_CHECK_BATCH", "2"))
        check_seq = int(os.getenv("E2E_CHECK_SEQ", "1"))
        check_seed = int(os.getenv("E2E_CHECK_SEED", "0"))
        check_enabled = os.getenv("E2E_CHECK_SPARSE_DENSE", "1") != "0"

        if check_enabled:
            torch.manual_seed(check_seed)
            layer = model.model.layers[check_layer_idx]
            proj_module = get_projection_module(layer, check_proj_name)

            entry = sparse_params[check_layer_idx][check_proj_name]
            K = int(entry["K"])
            x = torch.randn(check_batch, check_seq, K, device=DEVICE, dtype=torch.float32)
            x_2d = x.reshape(-1, K)

            pruned_w = entry["pruned_weight"].to(dtype=torch.float32)
            bias = proj_module.bias
            if bias is not None:
                bias = bias.to(dtype=torch.float32)

            y_dense = F.linear(x_2d, pruned_w.t().contiguous(), bias)

            y_sparse = nmsparse_module.forward(
                x_2d, entry["mat_data"], entry["mat_index"],
                entry["w"], entry["h"], entry["block_width"],
                NUM_THREADS, entry["vec_width"], x_2d.shape[0], x_2d.shape[1]
            )
            if bias is not None:
                y_sparse = y_sparse + bias

            diff = y_dense - y_sparse
            abs_diff = diff.abs()
            max_abs_err = abs_diff.max().item()
            mean_abs_err = abs_diff.mean().item()
            denom = y_dense.abs().clamp_min(1e-8)
            max_rel_err = (abs_diff / denom).max().item()

            meta = {
                "K": int(entry.get("K", x_2d.shape[1])),
                "N": int(entry.get("N", y_sparse.shape[1])),
                "w": int(entry["w"]),
                "h": int(entry["h"]),
                "block_width": int(entry["block_width"]),
                "vec_width": int(entry["vec_width"]),
            }
            print(
                "\n[CorrectnessCheck] sparse vs dense_pruned (FP32):"
                f" layer={check_layer_idx}, proj={check_proj_name}, x=[{check_batch},{check_seq},{K}]\n"
                f"  abs_err: max={max_abs_err:.6e}, mean={mean_abs_err:.6e}\n"
                f"  rel_err: max={max_rel_err:.6e} (|dense| clamp_min=1e-8)\n"
                f"  meta: K={meta['K']} N={meta['N']} w={meta['w']} h={meta['h']} "
                f"block_width={meta['block_width']} vec_width={meta['vec_width']}\n"
                f"  tensors: x={x_2d.dtype}/{x_2d.device}, pruned_w={pruned_w.dtype}/{pruned_w.device}, "
                f"mat_data={entry['mat_data'].dtype}/{entry['mat_data'].device}, "
                f"mat_index={entry['mat_index'].dtype}/{entry['mat_index'].device}, "
                f"bias={(str(bias.dtype) + '/' + str(bias.device)) if bias is not None else 'None'}\n"
                f"  tf32: matmul_allow_tf32={torch.backends.cuda.matmul.allow_tf32}, "
                f"cudnn_allow_tf32={torch.backends.cudnn.allow_tf32}, "
                f"matmul_precision={torch.get_float32_matmul_precision()}"
            )
    except Exception as e:
        print(f"\n[CorrectnessCheck] skipped due to error: {e}")
    ...
    sparse_prefill = os.getenv('E2E_SPARSE_PREFILL', '0') == '1'
    if sparse_prefill:
        apply_sparse_forwards(model, nmsparse_module, sparse_params, proj_names, None)
    ...
    print("✓ 稀疏化已应用")
    print(f"  - nmSPARSE: 7 个投影 × 32 层 = 128 个矩阵 ")
