...
# Isolated variant of llama.py: adds decode-token weighted Hessian for AR calibration.
# The original /wangqitong/sparsegpt-master_4/llama.py is intentionally unchanged.
...
def _safe_cholesky_with_wider_fallback(H, diag, upper=False, label="H"):
    """
    Runtime-only patch for this isolated AR-token-weighted script.
    Long decode Hessians can be PSD in theory but slightly indefinite in fp32.
    """
    last_error = None
    H = torch.nan_to_num(H, nan=0.0, posinf=0.0, neginf=0.0)
    H = 0.5 * (H + H.t())

    diag = diag.to(H.device)
    ...
    for attempt, jitter_scale in enumerate(jitter_scales):
        H_try = H
        if jitter_scale > 0:
            H_try = H.clone()
    ...
    try:
        min_eig = torch.linalg.eigvalsh(H).min()
        if torch.isfinite(min_eig) and min_eig < 0:
            shift = -min_eig + diag_abs_mean * 1e-4
        else:
            shift = diag_abs_mean * 1e-4
        H_try = H.clone()
        H_try[diag, diag] += shift
        chol = torch.linalg.cholesky(H_try, upper=upper)
        print(
            f"[WARN] {label} Cholesky used eigval fallback "
            f"shift={float(shift.item()):.6e}"
        )
        return chol
    except RuntimeError as err:
        last_error = err
...
def _decode_token_weight(pos, strategy, min_weight, max_weight, horizon):
    """Return alpha_k for zero-based decode token position k."""
    if strategy == "uniform":
        return 1.0

    if not (math.isfinite(min_weight) and math.isfinite(max_weight)):
        return 1.0

    horizon = max(2, int(horizon))
    pos = max(0, min(int(pos), horizon - 1))
    progress = pos / (horizon - 1)

    if strategy in ("log_decrease", "log_increase"):
        progress = math.log1p(pos) / math.log(horizon)

    if strategy in ("linear_decrease", "log_decrease"):
        return max_weight - (max_weight - min_weight) * progress
    if strategy in ("linear_increase", "log_increase"):
        return min_weight + (max_weight - min_weight) * progress

    raise ValueError(f"Unsupported token weight strategy: {strategy}")


def _add_weighted_token_normalized_batch(gpt, inp, weight):
    """
    Accumulate weighted Hessian:
      H = 2 / sum(alpha_k N_k) * sum(alpha_k X_k X_k^T).
    """
    if inp.dim() == 3:
        inp = inp.reshape((-1, inp.shape[-1]))
    elif inp.dim() == 1:
    ...
    old_weighted_samples = float(getattr(gpt, "weighted_nsamples", float(gpt.nsamples)))
    if not math.isfinite(old_weighted_samples) or old_weighted_samples < 0:
        old_weighted_samples = 0.0

    new_weighted_samples = old_weighted_samples + weight * tmp
    if not math.isfinite(new_weighted_samples) or new_weighted_samples <= 0:
        return

    if not torch.isfinite(gpt.H).all():
        gpt.H = torch.nan_to_num(gpt.H, nan=0.0, posinf=0.0, neginf=0.0)

    inp = inp.to(gpt.hessian_device)
    if not torch.isfinite(inp).all():
        inp = torch.nan_to_num(inp, nan=0.0, posinf=0.0, neginf=0.0)
    inp = inp.t()
    gpt.H *= old_weighted_samples / new_weighted_samples
    gpt.weighted_nsamples = new_weighted_samples
    gpt.nsamples = max(1, int(round(new_weighted_samples)))
    inp = math.sqrt(2 * weight / new_weighted_samples) * inp.float()
    gpt.H += inp.matmul(inp.t())
...
def llama_sequential_ar(model, prompts, tokenizer, dev, args):
    ...
    print(
        "AR token weighting: "
        f"strategy={token_weight_strategy}, min={token_weight_min}, "
        f"max={token_weight_max}, horizon={token_weight_horizon}"
    )
        ...
        def _hook(mod, inp, out):
            ...
            if token_weighting_enabled:
                alpha = _decode_token_weight(
                    decode_pos,
                    token_weight_strategy,
                    token_weight_min,
                    token_weight_max,
                    token_weight_horizon,
                )
        ...
        if last_k > 0:
            ...
            for key, buf in last_k_buffers.items():
                ...
                if token_weighting_enabled:
                    ...
                    x_batch = torch.cat([x_step for _, x_step in buf], dim=0)
                    if mix_prefill_decode:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ar_token_weight_strategy",
        type=str,
        default="uniform",
        choices=["uniform", "linear_decrease", "linear_increase", "log_decrease", "log_increase"],
        help="Decode-token Hessian weighting strategy for AR calibration. "
             "Use log_decrease to give earlier decode tokens larger Hessian weight.",
    )
    parser.add_argument(
        "--ar_token_min_weight",
        type=float,
        default=0.8,
        help="Minimum decode-token Hessian weight for non-uniform AR token weighting.",
    )
    parser.add_argument(
        "--ar_token_max_weight",
        type=float,
        default=1.2,
        help="Maximum decode-token Hessian weight for non-uniform AR token weighting.",
    )
    parser.add_argument(
        "--ar_token_weight_horizon",
        type=int,
        default=256,
        help="Decode-token position horizon used to map token index to min/max weight.",
    )
    ...
    if not (math.isfinite(args.ar_prefill_weight) and math.isfinite(args.ar_decode_weight)):
        parser.error("--ar_prefill_weight and --ar_decode_weight must be finite numbers.")
    ...
    if not (math.isfinite(args.ar_token_min_weight) and math.isfinite(args.ar_token_max_weight)):
        parser.error("--ar_token_min_weight and --ar_token_max_weight must be finite numbers.")
    if args.ar_token_min_weight < 0 or args.ar_token_max_weight < 0:
        parser.error("--ar_token_min_weight and --ar_token_max_weight must be non-negative.")
    if args.ar_token_max_weight < args.ar_token_min_weight:
        parser.error("--ar_token_max_weight must be >= --ar_token_min_weight.")
    if args.ar_token_weight_strategy != "uniform" and args.ar_token_max_weight <= 0:
        parser.error("--ar_token_max_weight must be positive for non-uniform token weighting.")
    if args.ar_token_weight_horizon <= 1:
        parser.error("--ar_token_weight_horizon must be greater than 1.")
    if args.ar_token_weight_strategy != "uniform" and not args.ar_decoding:
        parser.error("--ar_token_weight_strategy requires --ar_decoding unless set to uniform.")
    ...
    if (args.sparsity or args.prunen) and not args.gmp:
        ...
        if args.baseline_config_file:
            baseline_config = {
                ...
                "ar_token_weight_strategy": args.ar_token_weight_strategy,
                "ar_token_min_weight": args.ar_token_min_weight,
                "ar_token_max_weight": args.ar_token_max_weight,
                "ar_token_weight_horizon": args.ar_token_weight_horizon,
